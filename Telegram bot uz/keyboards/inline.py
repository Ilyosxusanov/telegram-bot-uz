...
def movie_keyboard(download_link, lang='uz'):
        ...
        if download_link and download_link.startswith("http"):
                    text = "* Filmni yuklab olish" if lang == 'uz' else ("* * Download Movie" if lang == 'ru' else "* Download Movie")
...
def category_movies_keyboard(movies):
        builder = InlineKeyboardBuilder()
    for movie in movies[:50]:
                title = movie[1]
                code = movie[5]
                rating = movie[6]
                builder.row(types.InlineKeyboardButton(
                    text=f"* {title} *{rating}", callback_data=f"movie_{code}")
                           )
            return builder.as_markup()
...
def lang_keyboard():
        builder = InlineKeyboardBuilder()
    builder.row(
                types.InlineKeyboardButton(text="* Uzbekcha", callback_data="setlang_uz"),
                types.InlineKeyboardButton(text="* Russian", callback_data="setlang_ru"),
...
def main_menu_keyboard(lang='uz'):
    ...
    builder.row(types.InlineKeyboardButton(text=t['by_code'], callback_data="cat_code"))
