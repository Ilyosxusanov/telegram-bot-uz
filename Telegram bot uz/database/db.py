...
def add_movie(title, description, image_url, download_link, code, imdb_rating=0.0):
    conn = sqlite3.connect('database/bot_db.sqlite')
    cursor = conn.cursor()
    try:
        cursor.execute('''
            INSERT INTO movies (title, description, image_url, download_link, code, imdb_rating) 
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (title, description, image_url, download_link, code, imdb_rating))
        conn.commit()
        return True
...
def get_all_movies():
    conn = sqlite3.connect('database/bot_db.sqlite')
    cursor = conn.cursor()
    cursor.execute("SELECT title, code, description, imdb_rating FROM movies")
    movies = cursor.fetchall()
    conn.close()
    return movies
...
def update_movie(code, title=None, description=None, image_url=None, download_link=None, imdb_rating=None):
    conn = sqlite3.connect('database/bot_db.sqlite')
    cursor = conn.cursor()
