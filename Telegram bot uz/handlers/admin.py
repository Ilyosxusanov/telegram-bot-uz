test from aiogram import Router, types, F, Bot
...
async def process_media(message: types.Message, state: FSMContext):
    ...
    if message.photo:
            ...
            return await message.answer("* Iltimos, video, rasm yoki link yuboring:", reply_markup=cancel_kb())
...
async def process_imdb(message: types.Message, state: FSMContext):
        ...
        return await message.answer("* Faqat raqam kiriting (masalan 8.5):", reply_markup=cancel_kb())
...
async def reply_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
                f"* ID: `{user_id}` bo'lgan foydalanuvchiga javobingizni yozing:",
                parse_mode="Markdown",
...
async def process_admin_reply(message: types.Message, state: FSMContext, bot: Bot):
    ...
    try:
            ...
            await bot.send_photo(target_user_id, message.photo[-1].file_id, caption=f"* **Admin javobi:**\n\n{message.caption or ''}", parse_mode="Markdown")
...
async def process_edit_code(message: types.Message, state: FSMContext):
    ...
    kb.row(types.InlineKeyboardButton(text="* Media (Video/Rasm)", callback_data="editfield_image_url"))
...
async def process_new_value(message: types.Message, state: FSMContext):
    ...
    if field == "image_url":
                ...
                return await message.answer("* Iltimos, media yuboring!")
elif field == "imdb_rating":
        try:
                        new_value = float(message.text)
                    except:
            return await message.answer("* Iltimos, IMDb reytingini raqamda yuboring (masalan: 8.5)!")
                    else:
    ...
    if success:
                ...
                kb.row(types.InlineKeyboardButton(text="* Yana tahrirlash", callback_data=f"edit_again_{code}"))
...
async def edit_again_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardBuilder()
    kb.row(types.InlineKeyboardButton(text="* Nomi", callback_data="editfield_title"))
    kb.row(types.InlineKeyboardButton(text="* Media", callback_data="editfield_image_url"))
    kb.row(types.InlineKeyboardButton(text="* Tavsif", callback_data="editfield_description"))
